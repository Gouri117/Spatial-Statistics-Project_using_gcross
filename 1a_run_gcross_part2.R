...
data_file <- '/nfs/turbo/umms-ukarvind/shared_data/mfIHC_collab_data/stanford_crc/subset_data_samples_with_outcome_split2.csv'
...
write.csv(g_auc_matrix, file.path(out_dir, "g_cross_full_matrix_by_window_split2.csv"), row.names = FALSE)

composition_df <- do.call(rbind, composition_long)
write.csv(composition_df, file.path(out_dir, "composition_by_window_split2.csv"), row.names = FALSE)

message("Finished SPLIT 2.")
